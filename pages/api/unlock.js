...
export default async function handler(req, res) {
  ...
  const PIN = "1234"; // As per the user's python script
  ...
  try {
    const body = JSON.stringify({ pin: PIN });
    const signature = crypto.HmacSHA256(body, PI_SERVO_SECRET).toString(crypto.enc.Hex);

    const response = await fetch(`${PI_SERVO_URL}unlock`, {
      method: 'POST',
      headers: {
    ...
    console.error('Error in /api/unlock:', error);
